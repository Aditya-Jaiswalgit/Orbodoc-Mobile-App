...
interface UpdateClinicMappingModalProps {
  visible: boolean;
  item: any | null;
  clinicName?: string;
  onClose: () => void;
...
export const UpdateClinicMappingModal: React.FC<UpdateClinicMappingModalProps> = ({
  visible,
  item,
  ...
  const [testPrice, setTestPrice] = useState('306');
  ...
  useEffect(() => {
    if (item) {
      setTestPrice(String(item.price ?? item.clinic_price ?? '306'));
      setDiscountPrice(String(item.discount_price ?? item.clinic_discount_price ?? '20'));
      setHomeCollection(
        item.home_collection_available === 1 || item.clinic_home_collection_available === 1
          ? 'Available'
          : 'Not available'
      );
      setClinicAvailability(
        item.is_available === 0 || item.clinic_is_available === 0
          ? 'Not available'
          : 'Available'
      );
    }
        ...
        <KeyboardAvoidingView
          ...
          <View style={styles.header}>
            <View style={styles.headerLeft}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.headerTitle}>Update clinic mapping</Text>
          ...
          <ScrollView
            ...
            <View style={styles.summaryCard}>
              <Text style={styles.summaryTitle}>{item?.test_name || '17-OH Progesterone'}</Text>
              <Text style={styles.summarySubtitle}>
                {item?.test_code || '17OHP'} • {item?.description || 'Measures 17-hydroxyprogesterone'}
              </Text>
              <View style={styles.pillsRow}>
                <View style={styles.pillAlreadyMapped}>
                  <Text style={styles.pillAlreadyMappedText}>Already mapped</Text>
                </View>
                <View style={styles.pillExistingPrice}>
                  <Text style={styles.pillExistingPriceText}>
                    Existing ₹{existingPriceVal.toFixed(2)}
                  </Text>
                </View>
              </View>
            </View>
          ...
          <View style={styles.footerRow}>
            ...
            <TouchableOpacity
              ...
              <Text style={styles.updateBtnText}>Update Mapping</Text>
            </TouchableOpacity>
          </View>
        </KeyboardAvoidingView>
      </View>
